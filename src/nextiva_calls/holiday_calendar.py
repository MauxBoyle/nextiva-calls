...
def parse_opm_icalendar(text: str) -> HolidayCalendar:
    ...
    entries = {holiday.day: holiday for holiday in events}
    if len(entries) != len(events):
        raise HolidayCalendarError("OPM calendar contains duplicate holiday dates")
...
def resolve_holiday_calendar(
    ...
    *,
    ...
    """Refresh OPM, or use a valid covering cache when refresh fails.

    The boolean is ``True`` when a cache was used, so callers can make that
    operational detail visible without weakening the coverage guarantee.
    """
